...
- make explicit that this is a protocol/design trade rather than a claim that a different syntactic Scala 2 transformer would have been impossible;
- replace the old `???` for manually wiring published marker/handler modules with the current documented `build.sbt` translation;
...
Parsing turns text into AST structure, but structural holes/splices still need separate substitution or rewriting:

```scala
c.parse(s"$left + $right") // produces a tree, `left` and `right` are strings
```

It's difficult to switch backward/forward between strings/trees

Quasiquotes make those holes part of the tree template itself:

```scala
q"$left + $right" // produces a tree, `left` and `right` are trees
```
...
The user-visible compiler-phase relationship was:
...
There is an implementation detail worth distinguishing from the phase diagram. Scala 2 Macro Paradise was loaded as an ordinary `scala.tools.nsc.plugins.Plugin`, but its plugin class had `components = Nil` and mixed in its `AnalyzerPlugins` support. It registered both:
...
# Scala 3 compiler-plugin taxonomy: standard vs research

Scala 3 documentation distinguishes two relevant plugin APIs:

```text
StandardPlugin
  -> contributes PluginPhase(s) to the compiler pipeline
  -> normal Scala 3 compiler-plugin mechanism

ResearchPlugin
  -> receives the whole compiler phase pipeline
  -> may add / remove / replace / rearrange phases
  -> intended for experimentation with the compiler pipeline itself
  -> documented as available only on nightly / snapshot Scala 3 compilers
```
...
## Why did Scala 2 Macro Paradise need analyzer/namer/typer integration while our Scala 3 design does not?

The short answer is: **the two systems use different expansion protocols**.

It is not that Scala 3 has somehow eliminated the need for name resolution or symbol entering. We deliberately arrange for those operations to happen *after* our transformation, on the final transformed `untpd` program.

Scala 2 Macro Paradise was built as an extension of Scala 2's macro/analyzer machinery. To expand `@foo`, Paradise relied on the analyzer lifecycle in several ways:

- the annotation and its macro implementation were represented through compiler symbols and macro bindings rather than through a separate pre-typer handler registry;
- expansion happened while the namer was entering stats, so generated definitions had to be entered into the same scope as part of that process;
- companion creation/lookup had to cooperate with the namer's companion machinery;
- `macroTransform` itself was a Scala macro implementation whose body/signature had to be handled by the macro typer machinery.

That is why the Scala 2 implementation registered hooks such as:
...
Our current Scala 3 design deliberately moves those prerequisites outside ordinary typer:

```text
precompiled marker class
  + runtime @expander("Handler") metadata
  + syntactic annotation identity
  + a bounded explicit-import resolver
  + explicit precompiled handler classpath
        |
        v
pre-typer ExpansionHandler invocation
        |
        v
validated untpd replacement / structured edit
        |
        v
ordinary Dotty typer enters symbols, resolves names and types the final program
```

This changes who owns several responsibilities.

### Annotation identity

Before typer, there is no general semantic symbol resolution available to us. The plugin therefore intentionally uses a bounded syntactic identity model. Qualified annotation names can be read directly from raw trees; a deliberately small explicit-import resolver handles the supported source-preceding import slice. It does **not** pretend to implement general Scala name resolution.

The broad/default external-handler path can then inspect the already compiled marker class metadata and discover the named precompiled handler without first typing the consumer's annotation application.

### Handler implementation

The handler is already compiled and loadable before the consumer reaches the plugin phase. It is not a `macroTransform` method that the current consumer compilation has to discover and type through macro machinery.

### Generated symbols

Handlers generate or rewrite **untyped syntax**. They are not expected to create the final typed symbols for generated definitions. Because the transformation finishes before typer begins, stock Dotty sees the transformed program as its input and creates/enters symbols, resolves references, overloads and types in the ordinary way.

This is an important simplification: instead of mutating a symbol table that has already been partially populated, we replace the raw program before that semantic work is authoritative.

### Companions

Macro-Paradise discovers and validates current primary/companion relationships structurally in the staged untyped program. A handler may request companion creation/replacement/merge, and after the edit the plugin recomputes the final relationships. The ordinary typer subsequently sees those companion definitions already present.

### The price of avoiding analyzer integration

This does **not** give us all the power of semantic analyzer hooks for free. The current contract is intentionally narrower:

- annotation identity resolution is bounded rather than full semantic resolution;
- handlers receive raw `untpd` plus bounded syntactic views, not general typed symbols/types;
- the broad/default architecture expects handlers to be precompiled;
- general same-file source handlers are not supported;
- targets are currently the bounded package-level Class/Trait/Object slice;
- transformations that genuinely require type information must happen differently or wait for a typed phase.

The bounded same-module experiment illustrates the boundary particularly well: once the handler itself lives in the same source module, the build/plugin has to suspend consumers, compile the selected handler source, and resume consumers with freshly compiled handler output. In other words, some of the complexity returns as soon as we relax the "precompiled before pre-typer expansion" assumption.

So a useful summary is:

```text
Scala 2 Paradise
  integrate expansion into the analyzer while symbols are being entered

current Scala 3 Macro-Paradise
  do a bounded syntactic transformation before typer
  -> let the ordinary typer perform semantic work once, on the transformed program
```

This is a design trade, not a theorem that Scala 2 could never have had a different purely syntactic pre-namer transformer. It explains why **Scala 2 Macro Paradise as actually designed** needed analyzer/macro hooks, while **our current Scala 3 protocol** can remain outside typer.
...
Current Macro-Paradise's plugin entry point extends Scala 3 `StandardPlugin`, and its expansion work is supplied as a `PluginPhase`. So **“standard Scala 3 compiler plugin” is technically correct in the Scala 3 API taxonomy**.

That does not mean its phase placement is ordinary. Most examples of standard plugins operate on typed trees after typer. Macro-Paradise deliberately schedules its phase **after `parser` and before `typer`** and rewrites untyped compilation-unit trees.

It is therefore useful to say both things:

> Macro-Paradise is implemented as a Scala 3 `StandardPlugin`.
>
> Its pre-typer use of that API is unusual and gives it some of the source-transformation power one might otherwise associate conceptually with analyzer/research-level compiler extension.

It is **not** a Scala 3 `ResearchPlugin`: it does not extend `ResearchPlugin`, and the project qualifies stable Scala releases (`3.3.8`, `3.8.4`, `3.9.0`), whereas Scala's documentation describes research plugins as nightly/snapshot-only.
...
# Walk by Scala tree representations

The original draft's diagram is useful as a map of the **walk we are taking from a public macro value toward compiler internals**.

The arrows in this diagram mean **directions of our walk / representation access**. They do **not** mean compiler phase order, subtype relationships, or that `tpd` compiles into `untpd`.
...
There is also Level 0 before the diagram: ordinary values / the macro call site.

For comparison, actual compiler phase direction is the other way around between untyped and typed trees:
...
So we keep both pictures:

- the **walk diagram** answers “how far down into implementation/compiler representations are we walking?”;
- the **phase diagram** answers “in what order does the compiler process source?”.
...
Or walk through branch (A) to internal `ExprImpl`:
...
Or walk through branch (B) using public quoted reflection:
...
## Level 2 (B): `q.reflect.Term` / `quotes.reflect.Term`
...
Or, as part of our walk toward the raw representation, construct an untyped tree and explicitly ask a typer to type it:
...
# What the sbt plugin actually does: annotations and handlers published (`0.2.0-SNAPSHOT` current protocol)

The first draft left this as `???`. The current external-handler authoring guide now has the explicit manual translation of:
...
For this no-AutoPlugin recipe, copy the public `ExternalArtifactIdentity.scala` helper from the Macro-Paradise external-handler starter into the downstream build as `project/ExternalArtifactIdentity.scala`.

Then the documented manual `build.sbt` is:
...
This makes the AutoPlugin/manual equivalence concrete:

- marker modules are ordinary compile-time dependencies; `% Provided` keeps marker-only artifacts off the application runtime classpath;
- handler modules live in a hidden tool-only `MacroParadiseHandler` configuration rather than ordinary application dependencies;
- direct handlers are resolved first and their canonical de-duplicated transitive closure is retained;
- `ExternalArtifactIdentity.combined(...)` covers the exact marker artifacts and the effective ordered handler classpath;
- the manual build emits the same three essential compiler inputs:
...
The larger executable `manual-published/build.sbt` fixture expands this to multiple markers/handlers and verifies direct-first ordering, exactly-once shared transitive runtime, runtime isolation, identity and compiler options.
...
- implemented as a Scala 3 `StandardPlugin`, not a `ResearchPlugin`;
...
- that Macro-Paradise is implemented as a Scala 3 `ResearchPlugin`;
...
2. Macro-Paradise's Scala 3 `StandardPlugin` schedules its expansion phase after parser and before typer.
